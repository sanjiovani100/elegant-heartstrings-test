...
const Showcase = () => {
  ...
  const handleExplore = useCallback((title: string) => {
    toast({
      title: "Exploring Collection",
      description: `You are exploring ${title}`,
    });
  }, [toast]);

  const handleDetails = useCallback((title: string) => {
    toast({
            ...
            <CarouselContent className="-ml-4 md:-ml-6">
              {showcaseItems.map((item, index) => (
                <CarouselItem key={index} className="pl-4 md:pl-6 md:basis-1/2 lg:basis-1/3">
                  <ShowcaseItem 
                    {...item} 
                    onExplore={() => handleExplore(item.title)}
                    onDetails={() => handleDetails(item.title)}
                  />
                </CarouselItem>
              ))}
            </CarouselContent>
